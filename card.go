...
//
// Note that while form annotations are used for updates, cards have some
// unusual logic on creates that necessitates manual handling of all
// parameters. See AppendToAsCardOrExternalAccount.
...
// AppendToAsCardOrExternalAccount appends the given CardParams as either a
...
func (c *CardParams) AppendToAsCardOrExternalAccount(body *form.Values, keyParts []string) {
	if len(c.Token) > 0 && len(c.Account) > 0 {
		...
		body.Add(form.FormatKey(append(keyParts, "source")), c.Token)

		if c.Default {
			body.Add(form.FormatKey(append(keyParts, "default_for_currency")), strconv.FormatBool(c.Default))
		}
	} else {
		body.Add(form.FormatKey(append(keyParts, "source", "object")), "card")
		body.Add(form.FormatKey(append(keyParts, "source", "number")), c.Number)

		if len(c.CVC) > 0 {
			body.Add(form.FormatKey(append(keyParts, "source", "cvc")), c.CVC)
		}

		if len(c.Currency) > 0 {
			body.Add(form.FormatKey(append(keyParts, "source", "currency")), c.Currency)
		}

		if c.Default {
			body.Add(form.FormatKey(append(keyParts, "source", "default_for_currency")), strconv.FormatBool(c.Default))
		}
	}

	if len(c.Month) > 0 {
		body.Add(form.FormatKey(append(keyParts, "source", "exp_month")), c.Month)
	}

	if len(c.Year) > 0 {
		body.Add(form.FormatKey(append(keyParts, "source", "exp_year")), c.Year)
	}

	if len(c.Name) > 0 {
		body.Add(form.FormatKey(append(keyParts, "source", "name")), c.Name)
	}

	if len(c.Address1) > 0 {
		body.Add(form.FormatKey(append(keyParts, "source", "address_line1")), c.Address1)
	}

	if len(c.Address2) > 0 {
		body.Add(form.FormatKey(append(keyParts, "source", "address_line2")), c.Address2)
	}

	if len(c.City) > 0 {
		body.Add(form.FormatKey(append(keyParts, "source", "address_city")), c.City)
	}

	if len(c.State) > 0 {
		body.Add(form.FormatKey(append(keyParts, "source", "address_state")), c.State)
	}

	if len(c.Zip) > 0 {
		body.Add(form.FormatKey(append(keyParts, "card", "address_zip")), c.Zip)
	}

	if len(c.Country) > 0 {
		body.Add(form.FormatKey(append(keyParts, "card", "address_country")), c.Country)
	}
}
